          ...
          {newsList.map((news) => (
            <Link key={news.id} href={`/news/${news.id}`} className="group">
              <div className="bg-white rounded-xl shadow-md p-0 border-l-4 border-orange-400 flex flex-col h-full overflow-hidden transition-transform group-hover:-translate-y-1 group-hover:shadow-lg">
                ...
                <div className="p-5 flex flex-col min-h-[140px] opacity-100">
                  <span className="text-xs text-gray-400 mb-1 opacity-100">{news.date}</span>
                  <h3 className="text-lg font-semibold text-gray-900 mb-2 line-clamp-2 opacity-100">{news.title}</h3>
                  <p className="text-gray-700 text-sm mb-2 line-clamp-3 opacity-100">{news.excerpt}</p>
                  <span className="text-orange-500 text-xs mt-auto font-bold group-hover:underline opacity-100">続きを読む →</span>
                </div>
              </div>
            </Link>
          ))}
