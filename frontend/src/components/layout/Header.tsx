  ...
  return (
    ...
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled || isMenuOpen || !isHomePage ? 'bg-white/90 backdrop-blur-md shadow-sm py-3' : 'bg-transparent py-5'
      }`}
    >
            ...
            {navLinks.map((link) => (
              <Link 
                ...
                className={`text-sm font-medium transition-colors hover:text-orange-500 ${
                  isScrolled || isMenuOpen || !isHomePage ? 'text-gray-700' : 'text-white/90 hover:text-white'
                }`}
              >
                {link.label}
              </Link>
          ...
          >
            <div className="w-6 h-5 relative flex flex-col justify-between">
              <span className={`w-full h-0.5 rounded-full transition-all duration-300 ${
                isMenuOpen ? 'rotate-45 translate-y-2 bg-gray-900' : (isScrolled || !isHomePage ? 'bg-gray-900' : 'bg-white')
              }`} />
              <span className={`w-full h-0.5 rounded-full transition-all duration-300 ${
                isMenuOpen ? 'opacity-0' : (isScrolled || !isHomePage ? 'bg-gray-900' : 'bg-white')
              }`} />
              <span className={`w-full h-0.5 rounded-full transition-all duration-300 ${
                isMenuOpen ? '-rotate-45 -translate-y-2.5 bg-gray-900' : (isScrolled || !isHomePage ? 'bg-gray-900' : 'bg-white')
              }`} />
            </div>
          </button>
        </div>
